...
マネックス証券（ON COMPASS）口座のデータも、ローカル API 接続時に Web の設定画面（マネックス証券 一括取り込み）から貼り付けで登録・更新します。DB 格納の code 慣例は [dev/sql/monex/README.md](dev/sql/monex/README.md) を参照してください。投入後は `npm run pages:export` で静的 JSON を更新できます。

楽天証券口座のデータも、ローカル API 接続時に Web の設定画面（楽天証券一括取り込み）から貼り付けで登録・更新します。DB 格納の code 慣例は [dev/sql/rakuten/README.md](dev/sql/rakuten/README.md) を参照してください。
...
| データ投入 | iDeCo 貼り付け一括取り込み、マネックス証券 貼り付け一括取り込み、楽天証券 貼り付け一括取り込み、設定画面での銘柄・明細・指標登録 |
...
- 投資シミュレーション
- 通貨別分析（外貨建て資産の換算・集計）
- NISA・課税口座など他口座種別のデータ取り込み（マネックス証券・楽天証券は貼り付け取り込み対応済み）
- PostgreSQL への移行
