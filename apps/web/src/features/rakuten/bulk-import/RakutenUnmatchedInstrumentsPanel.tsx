...
type RakutenUnmatchedInstrumentsPanelProps = {
...
export function RakutenUnmatchedInstrumentsPanel({
  candidates,
  instruments,
  disabled,
  onCreate,
}: RakutenUnmatchedInstrumentsPanelProps) {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {candidates.map((candidate) => {
            ...
            const candidateKey = `${candidate.ticker ?? ""}:${candidate.instrumentName}:${candidate.accountId}`;
            let row = (
              <div
                key={candidateKey}
