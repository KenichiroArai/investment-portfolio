...
type RakutenBulkImportDraftTableProps = {
  drafts: RakutenHoldingDraftRow[];
  instruments: PasteInstrumentDto[];
  disabled: boolean;
  onChange: (drafts: RakutenHoldingDraftRow[]) => void;
};
...
export function RakutenBulkImportDraftTable({
  drafts,
  instruments,
  disabled,
  onChange,
}: RakutenBulkImportDraftTableProps) {
  let result = (
    <Table>
      <TableHeader>
        <TableRow>
                      ...
                      {instruments.map((instrument) => {
                        const accountSuffix =
                          instrument.accountId && instrument.accountId.trim() !== ""
                            ? ` [${instrument.accountId.replace(/^rakuten:/, "")}]`
                            : "";
                        let item = (
                          <SelectItem key={instrument.id} value={instrument.id}>
                            {instrument.ticker
                              ? `${instrument.ticker} / ${instrument.name}${accountSuffix}`
