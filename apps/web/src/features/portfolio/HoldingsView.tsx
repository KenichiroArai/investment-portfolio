...
type HoldingsViewMode = "range" | "compare";
...
const HOLDINGS_VIEW_MODE_LABELS: Record<HoldingsViewMode, string> = {
  range: "期間明細一覧",
  compare: "スナップショット比較",
};

function readHoldingsViewMode(value: string | null): HoldingsViewMode {
  let result: HoldingsViewMode = "range";
...
export function HoldingsView({ portfolioCode }: HoldingsViewProps) {
  ...
  const holdingsViewMode = readHoldingsViewMode(searchParams.get("view"));
  ...
  const setHoldingsViewMode = (mode: HoldingsViewMode) => {
  ...
  useEffect(() => {
    ...
    return result;
  }, [portfolioCode, comparisonDate, holdingsViewMode]);
  ...
  useEffect(() => {
    ...
    return result;
  }, [portfolioCode, rangeDatesKey, holdingsViewMode]);
  ...
  result = (
    <PageContainer>
      ...
      />
      <Tabs
        value={holdingsViewMode}
        onValueChange={(value) => {
          setHoldingsViewMode(readHoldingsViewMode(value));
        }}
      >
        <TabsList>
          {(Object.keys(HOLDINGS_VIEW_MODE_LABELS) as HoldingsViewMode[]).map(
            (mode) => {
              let trigger = (
                <TabsTrigger key={mode} value={mode}>
                  {HOLDINGS_VIEW_MODE_LABELS[mode]}
                </TabsTrigger>
              );
              return trigger;
            },
