...
貼り付け内訳に銘柄別の購入金額・損益は無いため、**商品分類ごと**に購入金額を汎用指標として登録し、同一 `account_id` 内の評価額比率で `book_value_minor` と損益メトリクスへ按分する。

### UI から設定（推奨）

1. 設定 → データ管理 → 「SBIラップ一括取り込み」で保有明細を登録する
2. 同画面の「汎用指標」タブで、商品ごとの購入金額（例: `AI投資 購入金額`）を登録する
3. 購入金額は **読み取り時** に有効な汎用指標から評価額比率で按分して表示される（保有明細を日付ごとに再登録する必要はない）

商品ごとの購入金額はスナップショット汎用指標（例: `sbi_wrap_product_cost_ai_investment`）に保持する。汎用指標は **登録した基準日から、同じ指標が次に更新されるまで有効**（例: 7/22 のみ登録すれば、その後の 8/2 などの保有日にも適用）。指標の登録・更新は明細を複製せず、指標行だけを書き込む。

### 開発用 seed（フォールバック）

全商品を固定 10,000 円で按分したい場合:
