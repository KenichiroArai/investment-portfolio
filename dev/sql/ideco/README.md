...
`明細.csv` には **複数の基準日** を含められます。各行の「日付」列でスナップショットがグループ化され、日付ごとに `portfolio_snapshots` へ upsert されます。再インポート時は同一基準日のスナップショットが更新され、最大の基準日が `is_current = 1` になります。

- `汎用.csv`（拠出金累計など）は日付列がないため、**最新基準日のスナップショットのみ** に付与されます
- 過去基準日では holdings から算出できる指標（評価額・簿価ベース損益など）を表示します
