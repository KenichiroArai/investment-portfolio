...
スナップショットごとに `code` は一意です。口座非依存の EAV パターンで、口座別の意味付けは code 慣例で区別します。読み取り時は、同じ `code` について **登録した基準日から次の更新まで有効** として、指定日以前の最新値が解決されます（削除は `text_value = __CLEARED__` で記録）。指標だけの更新は明細を複製しません。SBIラップの商品購入金額は、有効な汎用指標から読み取り時に明細の購入金額へ按分します。
