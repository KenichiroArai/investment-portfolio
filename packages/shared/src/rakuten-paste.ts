...
import { parseRakutenPagePaste } from "./rakuten-page-paste";
import {
  finishRakutenPasteRow,
  resolveRakutenAccountFieldsForTest as resolveAccountFieldsForTest,
...
export type { ParseRakutenPasteResult, RakutenHoldingPasteRow } from "./rakuten-paste-row";
export { finishRakutenPasteRow } from "./rakuten-paste-row";
...
function finishRow(params: {
  ...
  let result = finishRakutenPasteRow(params);
  return result;
}
...
export function parseRakutenPaste(content: string): ParseRakutenPasteResult {
  ...
  const format = detectRakutenPasteFormat(lines);
  if (format === "page") {
    result = parseRakutenPagePaste(lines);
    return result;
  }
...
export function resolveRakutenAccountFieldsForTest(rawAccountType: string) {
  let result = resolveAccountFieldsForTest(rawAccountType);
  return result;
}
